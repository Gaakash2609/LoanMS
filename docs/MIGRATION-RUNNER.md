...
| `scripts/1-build-and-push-migration-image.ps1` | Build + push image to ECR — **prep only** |
| `scripts/2-register-migration-task-definition.ps1` | Registers the task def — **prep only** |
| `scripts/3-RUN-MIGRATION-run-task.ps1` | **Actually runs the migration** — asks for typed confirmation |
...
1. **Secret JSON key name.** `ecs-task-def-migration.json` references
   ...
   used for `loanms/prod/db` in `docs/aws-ecs-cicd.md`. If `loanms/staging/db`
...
- `scripts/3-RUN-MIGRATION-run-task.ps1` requires typing `RUN MIGRATION`
