...
// ── Additional Bank Statement fallback (Rule 3.2 / Last Transaction failures only) ──
// Stored on `window` (not `let`) so it naturally resets on every fresh popup
// open (pfv9Open reloads this iframe's src), yet survives the per-file resets
// that handleFiles() below performs when the user uploads a replacement file.
window._pfLastTxnAttempts = window._pfLastTxnAttempts || 0;
window._pfStaleFailure    = window._pfStaleFailure || null; // {isStale, attempts, exhausted}
...
function extractAccountInfo(text) {
  ...
  for (const line of lines) {
    ...
    // ICICI-style: name stated unlabelled, immediately before "Your Base Branch:"
    if (!accountInfo.name) {
  ...
  // NOTE: a naive full-text IFSC fallback (matching any [A-Z]{4}0[A-Z0-9]{6}
  // shape anywhere in the document) was removed here — it was picking up
  // counterparty bank IFSC codes from transaction narrations (e.g. an IMPS/
  // NEFT transfer's recipient bank) instead of the account holder's own
  // IFSC, since transaction text dominates the document and isn't
  // distinguishable from a genuine header IFSC by shape alone. Only the
  // strict, explicitly-labelled match above is trusted; if the statement's
  // own header doesn't contain a labelled IFSC, this is correctly left
  // blank rather than showing a wrong value.
  if (!accountInfo.mobile) {
...
function parseTransactions(text, filename) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Strict, decimal-required amount matching only — matches the reference
    // implementation exactly. A prior fallback here (matching bare 4+ digit
    // numbers with no decimal/comma requirement) was misinterpreting account
    // numbers, UPI reference numbers, and other non-monetary digit sequences
    // as transaction amounts whenever a continuation-window line lacked a
    // clean decimal amount — removed, not replaced with new logic.
    const amounts = [...combined.matchAll(/[\d,]+\.\d{2}/g)].map(m => parseFloat(m[0].replace(/,/g,'')))
      // Sanity ceiling: reject anything above ₹100 crore — no genuine
      // transaction/balance in a personal statement reaches this size, so
      // treating such a match as a real amount would only ever be a parsing
      // artifact. Prevents any future artifact from silently becoming a
      // displayed "dummy" amount, rather than trying to guess its cause.
...
function finalizeProcessing() {
  if (!allTxns.length) {
    ...
    window._pfStaleFailure = null; // not a Rule-3.2 failure — Additional Statement flow doesn't apply
    return;
  }
  ...
  // Additional Bank Statement fallback: up to 3 upload attempts. Confirm Attachment
  // stays hidden (retry prompt shown instead) until either a fresh statement passes,
  // or all 3 attempts are exhausted — at which point Confirm Attachment reappears and
  // the application gets flagged for manual review (handled by the parent app).
  if (staledays > 7) {
    window._pfLastTxnAttempts = (window._pfLastTxnAttempts || 0) + 1;
    var _pfExhausted = window._pfLastTxnAttempts >= 3;
    window._pfStaleFailure = {
      isStale: true,
      attempts: window._pfLastTxnAttempts,
      exhausted: _pfExhausted
    };
    showBanner('error',
      ...
      (_pfExhausted
        ? `All 3 attempts used — this application will be flagged for manual review.`
        : `Attempt ${window._pfLastTxnAttempts}/3 — please upload an additional/more recent bank statement.`)
    );
    log('warn', `✗ REJECTED [Rule 3.2]: Last txn ${fmtDate(lastDate)} is ${staledays} day(s) old (max 7 allowed) — attempt ${window._pfLastTxnAttempts}/3`);
    hideLoader();
    updateStats(firstDate, lastDate, daysDiff, allTxns.length, 0, staledays);
    setStep(2);
    return;
  ...
  window._pfStaleFailure = null; // fresh statement passed — clear the fallback gate
...
function resetAll() {
  ...
  window._pfLastTxnAttempts = 0; window._pfStaleFailure = null; window._pfManualReviewRequired = false; window._pfAllUploadedFiles = []; // full manual reset also clears the Additional Statement gate
