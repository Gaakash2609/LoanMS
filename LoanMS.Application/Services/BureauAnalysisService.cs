...
{
    public interface IBureauAnalysisService
    {
        Task<BureauReportDetailDto> AnalyzeBureauReport(BureauReport report);
        Task<RiskAnalysisDto> CalculateRiskAnalysis(BureauReport report);
        Task<BehaviourAnalysisDto> AnalyzeCreditBehaviour(BureauReport report);
        Task<EnquiryAnalysisDto> AnalyzeEnquiries(BureauReport report);
        Task<List<string>> GenerateAutoInsights(BureauReport report);
    }

    public class BureauAnalysisService : IBureauAnalysisService
    {
        // Score Ranges
        ...
        public async Task<BureauReportDetailDto> AnalyzeBureauReport(BureauReport report)
        {
            var dto = new BureauReportDetailDto
            {
                Id = report.Id,
                ...
                CreditScore = new CreditScoreDto
                {
                    ...
                    PositiveFactors = report.PositiveFactors?.Select(f => new ScoreFactorDto
                    {
                        Factor = f.Factor,
                        ImpactScore = f.ImpactScore,
                        Description = f.Description
                    }).ToList() ?? new List<ScoreFactorDto>(),
                    NegativeFactors = report.NegativeFactors?.Select(f => new ScoreFactorDto
                    {
                        Factor = f.Factor,
                        ImpactScore = f.ImpactScore,
                        Description = f.Description
                ...
                CustomerProfile = new CustomerProfileDto
                {
                    ...
                    Addresses = report.Addresses?.Select(a => new AddressDto
                    {
                        Type = a.AddressType,
                        Street = a.Street,
                        City = a.City,
                    ...
                    EmploymentHistory = report.EmploymentHistory?.Select(e => new EmploymentDto
                    {
                ...
                // Account Summary
                AccountSummary = new AccountSummaryDto
                {
                ...
                // Accounts
                Accounts = report.Accounts?.Select(a => new BureauAccountDto
                {
                    Id = a.Id,
                ...
                }).ToList() ?? new List<BureauAccountDto>(),
        ...
        public async Task<RiskAnalysisDto> CalculateRiskAnalysis(BureauReport report)
        {
            ...
            var riskAnalysis = new RiskAnalysisDto
            {
                RiskLevel = riskLevel,
                RiskGrade = riskGrade,
        ...
        public async Task<BehaviourAnalysisDto> AnalyzeCreditBehaviour(BureauReport report)
        {
            ...
            return new BehaviourAnalysisDto
            {
        ...
        public async Task<EnquiryAnalysisDto> AnalyzeEnquiries(BureauReport report)
        {
            ...
            return new EnquiryAnalysisDto
            {
                ...
                EnquiryDetails = report.Enquiries?.Select(e => new EnquiryDto
                {
                    EnquiryDate = e.EnquiryDate,
                    EnquiryType = e.EnquiryType,
                    RequestedAmount = e.RequestedAmount,
                    Purpose = e.Purpose
                }).ToList() ?? new List<EnquiryDto>()
            };
        }
        ...
        private string GetRiskGrade(int creditScore, decimal riskScore)
        {
            if (creditScore >= 750 && riskScore < 30)
                return "A";
            else if (creditScore >= 650 && riskScore < 50)
                return "B";
            else if (creditScore >= 550 && riskScore < 70)
                return "C";
            else if (creditScore >= 300 && riskScore < 85)
                return "D";
            else
                return "E";
        }
        ...
        private List<RiskFactorDto> GetRiskFactors(BureauReport report)
        {
        ...
        private async Task<PaymentHistoryDto> AnalyzePaymentHistory(BureauReport report)
        {
            var history = report.PaymentHistory?.OrderBy(p => p.ReportMonth).ToList() ?? new List<BureauPaymentHistory>();

            return new PaymentHistoryDto
            {
                Monthly = history.Select(h => new MonthlyPaymentStatusDto
                {
                ...
                DelinquencyTracker = new DelinquencyTrackerDto
                {
